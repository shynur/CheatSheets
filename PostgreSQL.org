...
Local Variables:
coding: utf-8-unix
eval: (browse-url-default-browser "file://localhost/D:/Desktop/ToRead/SQL基础教程-MICK/SQL基础教程.pdf")
eval: (w32-notification-close (w32-notification-notify :body " " :title "读到了第 98 页"))
eval: (find-file-noselect "D:/Desktop/ToRead/SQL基础教程-MICK/src_ans/Answer/")
eval: (make-variable-buffer-local 'org-mode-abbrev-table)
eval: (abbrev-table-put org-mode-abbrev-table :parents
                        `(,sql-mode-abbrev-table
                          ,@(abbrev-table-get org-mode-abbrev-table :parents)))
                          eval: (define-abbrev org-mode-abbrev-table
        "begsrc" "#+BEGIN_SRC sql\n#+END_SRC"
        (lambda ()
          (previous-line 1)
          (end-of-line)))
eval: (abbrev-mode)
...
org-babel-default-header-args:sql: ((:engine . "postgresql")
                                    ...
                                    (:dbuser . "postgres")
                                    (:database . "shop"))
eval: (setenv "PGPASSWORD" " ")
eval: (add-hook 'before-save-hook (lambda () (org-babel-remove-result-one-or-many t)) -99 "buffer local")
eval: (company-mode -1)
eval: (eldoc-mode -1)
End:
