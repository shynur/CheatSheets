...
常用的有 ~AVG~, ~SUM~, ~MIN~, ~MAX~, 和 ~COUNT~ 这些.
...
eval: (w32-notification-close
       (w32-notification-notify :title "读到了第 100 页"
                                :body " "))
eval: (require 'ob-sql)
...
org-babel-default-header-args:sql: ((:engine . "postgresql")
                                    ...
                                    (:dbuser . "postgres")
                                    (:database . "shop"))
eval: (setenv  ; and modify the definition of ‘org-babel-execute:sql’ if you use MS-Windows.
       "PGPASSWORD"
       " ")
