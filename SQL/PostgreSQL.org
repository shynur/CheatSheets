...
- 差集 ~EXCEPT~
运算.
可以想象成将两张表<<纵向拼接>>纵向拼接的 \( 3 \) 种方式[fn:纵横拼接: A4 纸​_上下对折_, 上半张填入 =table_1=, 下半张填入 =table_2=, 就像 emoji‘⊟’, 此为纵向拼接.  同理, 横向拼接就是​_左右对折_, 就像 emoji‘◫’.], 因此, 需要保证两张表的结构相同[fn:: (TODO) 对列名似乎没有要求?]:
- 列数相同;
- 对应列的数据类型一致[fn:: 如果类型不同, 此时有些 RDBMS 会进行隐式类型转换.].

和数学中的集合运算一样, 运算结果中
- _不包含重复行, 除非在​/集合运算符/​后书写 ~ALL~ 关键字._
...
集合运算中的除法通常称为 /Division Operation in Relational Algebra/, SQL 并没有定义类似地运算符.
...
#+BEGIN_SRC emacs-lisp  :eval never
  ...
  eval: (w32-notification-close
         (w32-notification-notify :title "读到了第 276 页"
                                  :body " "))
  eval: (browse-url-default-browser "./SQL基础教程-MICK/TextBook.pdf")
  eval: (require 'ob-sql)
